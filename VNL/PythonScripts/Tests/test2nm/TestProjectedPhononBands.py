...
# Choose file
myfile = 'DynamicalMatrix.hdf5'  # 'TestPhonons1sided_1HBN_ABstackinga=2dot49.nc'
...
# Choose q-points
from NL.Analysis.BaseBandstructure import determinePath, combinedRoutes
print 'Symmetry points: ', configuration.bravaisLattice().symmetryPoints()
myroute = ['G', 'M', 'K', 'G']  # ['L', 'G', 'X', 'U', 'K', 'G']
route = combinedRoutes(myroute)
symmetry_points = configuration.bravaisLattice().symmetryPoints()
numpoints = 100
fractional_qpoints = determinePath(
    symmetry_points=symmetry_points, route=route, number_of_points=numpoints)
...
plotmode = 1
if plotmode == 0:
    scatter(numpy.repeat(bs._axisGUI(), n_modes), frequency_list.inUnitsOf(
        eV).flatten(), c=myscale, s=150, marker='o', edgecolor='none', cmap=cmap, norm=norm)
elif plotmode == 1:
    scatter(numpy.repeat(bs._axisGUI(), n_modes), frequency_list.inUnitsOf(eV).flatten(
...
filesave = 'g_membrane_displacement.nc'
