...
Zeta = 1.0 # Zeta = 0.31, 1.0, 10 for graphite, Si coupling (In SiO2) or something extremely strongly interacting, respectively.
...
lj_interlayer_potential.addParticleType(ParticleType.fromElement(Carbon, sigma=3.326*Angstrom, epsilon=Zeta*8.909*meV))
# Add Lennard-Jones potentials between the carbon atoms of different layers.
lj_interlayer_potential.addPotential(LennardJonesPotential('C', 'C', r_cut=10.0 * Angstrom))
lj_interlayer_potential.setTags(['Layer1', 'Layer2'])

# Combine all 3 potential sets in a single calculator.
calculator = TremoloXCalculator(parameters=[potentialSet_layer1, potentialSet_layer2, lj_interlayer_potential])
